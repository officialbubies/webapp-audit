...
async function scanUrl(url, options = {}) {
  ...
  const findings = [];
  const { timeout = 30000, deep = false } = options; // Default 30s timeout
  ...
  await detectAndScanApis(url, response.data, axiosConfig, findings);
...
async function detectAndScanApis(baseUrl, body, axiosConfig, findings) {
  if (typeof body !== 'string') return;
